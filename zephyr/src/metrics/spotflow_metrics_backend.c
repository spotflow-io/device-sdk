	...
	uint8_t max_labels)
{
	...
	if (max_labels > CONFIG_SPOTFLOW_METRICS_MAX_LABELS_PER_METRIC) {
		LOG_ERR("Invalid max_labels: %u (max %d)",
			max_labels, CONFIG_SPOTFLOW_METRICS_MAX_LABELS_PER_METRIC);
		return NULL;
	}
	...
	if (slot < 0) {
		LOG_ERR("Metric registry full (%d/%d)",
	...
	LOG_INF("Registered metric '%s' (type=%s, agg=%d, max_ts=%u, max_labels=%u)",
		normalized_name,
		(type == SPOTFLOW_METRIC_TYPE_INT) ? "int" : "float",
		metric->agg_interval,
		max_timeseries,
		max_labels);
	...
	uint8_t label_count)
{
	...
	if (label_count == 0 || label_count > base->max_labels) {
		LOG_ERR("Invalid label_count: %u (max %u)",
			label_count, base->max_labels);
		return -EINVAL;
	}
