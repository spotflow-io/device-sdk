...
/* Runtime packaging also permits transient format strings. */
...
ZTEST(log_cbor, test_embedded_format_fallback)
{
	uint8_t storage[sizeof(struct log_msg) + 128] __aligned(Z_LOG_MSG_ALIGNMENT) = { 0 };
	struct log_msg* msg = (void*)storage;
	const char format[] = "runtime %u";
	int len = cbprintf_package(msg->data, 128, 0, format, 19u);
	zassert_true(len > 0);
	struct cbprintf_package_hdr_ext* hdr = (void*)msg->data;
	size_t end = hdr->hdr.desc.len * sizeof(int);
	hdr->hdr.desc.str_cnt = 1;
	msg->data[end++] = offsetof(struct cbprintf_package_hdr_ext, fmt) / sizeof(int);
	memcpy(msg->data + end, format, sizeof(format));
	msg->hdr.desc.package_len = end + sizeof(format);
