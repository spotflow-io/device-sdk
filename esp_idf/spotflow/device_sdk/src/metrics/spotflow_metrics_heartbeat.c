...
/* ---------------------------------------------------------------------------
 * Timer callback - generates heartbeat and overwrites pending buffer
 * ---------------------------------------------------------------------------
 */
...
void spotflow_metrics_heartbeat_init(void)
{
    ...
    SPOTFLOW_LOG("Heartbeat initialized (interval=%d s)", CONFIG_SPOTFLOW_METRICS_HEARTBEAT_INTERVAL);
}
...
int spotflow_poll_and_process_heartbeat(void)
{
    ...
    for (int retry = 0; retry <= 3; retry++) {
        rc = spotflow_mqtt_publish_message(
            SPOTFLOW_MQTT_LOG_TOPIC,
            msg->payload,
            msg->len,
            SPOTFLOW_MQTT_LOG_QOS
        );

        if (rc != -EAGAIN) break; // published or failed with real error

        if (retry < 3) {
            SPOTFLOW_DEBUG("MQTT busy, retrying heartbeat in %d ms...", retry_delays_ms[retry]);
